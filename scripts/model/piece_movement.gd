			...
			if distance > 0:
				for i in range(distance):
					...
					if is_blocked(square):
						...
						moves.append(ChessPiece.Move.new(current_square.piece, current_square, square))
			else:
				...
				while next_square != null:
					if is_blocked(next_square):
						...
						moves.append(ChessPiece.Move.new(current_square.piece, current_square, next_square))
